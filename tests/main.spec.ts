...
describe("main.fc contract test", () => {
  ...
  it("should successfuly increase counter in contract and get the proper most recent sender address", async () => {
    console.log(
      `initWallet - ${initWallet.address} ownerWallet - ${ownerWallet.address}`
    );
    ...
    console.log(`Recent sender - ${data.recent_sender}`);
  ...
  it("successfully deposits funds", async () => {
    ...
    const balanceReques = await myContract.getBalance();
    console.log("Balance: " + balanceReques.balance);

    expect(balanceReques.balance).toBeGreaterThan(toNano("4.99"));
  });
  ...
  it("should return deposit funds as no command is sent", async () => {
    ...
    expect(balanceReques.balance).toEqual(0);
  });
  ...
  it("successfully withdraws funds on behalf of owner", async () => {
    ...
    // console.log(`Deposit result: ${depositMessageresult.result}`);
  ...
  it("fails to withdraw funds on behalt of non-owner", async () => {
    const senderWallet = await blockchain.treasury("sender");
    await myContract.sendDeposit(senderWallet.getSender(), toNano("5"));
